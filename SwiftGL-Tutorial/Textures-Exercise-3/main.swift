...
let alphaStep: Float = 0.1
let alphaUpperLimit: Float = 1.0
let alphaLowerLimit: Float = 0.0

func keyCallback(window: OpaquePointer!, key: Int32, scancode: Int32, action: Int32, mode: Int32) {
    if key == GLFW_KEY_ESCAPE && action == GLFW_PRESS {
        glfwSetWindowShouldClose(window, GL_TRUE)
    } else if key == GLFW_KEY_UP && action == GLFW_PRESS && textureAlpha < alphaUpperLimit {
        textureAlpha += alphaStep
    } else if key == GLFW_KEY_DOWN && action == GLFW_PRESS && textureAlpha > alphaLowerLimit {
        textureAlpha -= alphaStep
    }
}
...
func main() {
    ...
    while glfwWindowShouldClose(window) == GL_FALSE {
        ...
        glUniform1f(glGetUniformLocation(ourShader.program, "ourTexture2Alpha"), textureAlpha)
