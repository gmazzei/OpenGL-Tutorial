...
"}\n"
let fragmentOrangeShaderSource = "#version 330 core\n" +
    "out vec4 color;\n" +
    "void main()\n" +
    "{\n" +
...
let fragmentYellowShaderSource = "#version 330 core\n" +
    "out vec4 color;\n" +
    "void main()\n" +
    "{\n" +
...
let fragmentShaderSource = [fragmentOrangeShaderSource, fragmentYellowShaderSource]
...
func main() {
    ...
    var shaderProgram: [GLuint] = [0, 0]
    
    // Fragment Shaders
    for i in 0..<fragmentShaderSource.count {
        let fragmentShader = glCreateShader(type: GL_FRAGMENT_SHADER)
        fragmentShaderSource[i].withCString {
            var s = [$0]
        ...
        guard success == GL_TRUE else {
            glGetProgramInfoLog(shaderProgram[i], 512, nil, &infoLog)
            fatalError(String(cString:infoLog))
        }
