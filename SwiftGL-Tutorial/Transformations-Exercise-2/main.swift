...
func main() {
    ...
    while glfwWindowShouldClose(window) == GL_FALSE {
        ...
        // Box 1
        var transform1 = mat4()
        transform1 = SGLMath.translate(transform1, vec3(0.5, -0.5, 0.0))
        transform1 = SGLMath.rotate(transform1, Float(glfwGetTime()), vec3(0.0, 0.0, 1.0))
        
        let transformLoc1 = glGetUniformLocation(ourShader.program, "transform")
        withUnsafePointer(to: &transform1, {
            $0.withMemoryRebound(to: GLfloat.self, capacity: 4 * 4) {
                glUniformMatrix4fv(transformLoc1, 1, false, $0)
            }
        })
        ...
        let rotation = Float(sin(glfwGetTime()))
        transform2 = SGLMath.scale(transform2, vec3(rotation, rotation, 1.0))
        
        let transformLoc2 = glGetUniformLocation(ourShader.program, "transform")
        withUnsafePointer(to: &transform2, {
            $0.withMemoryRebound(to: GLfloat.self, capacity: 4 * 4) {
                glUniformMatrix4fv(transformLoc2, 1, false, $0)
            }
        })
